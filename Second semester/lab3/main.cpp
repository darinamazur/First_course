...
struct index_List{
    ...
    void to_index_columns_list(NodeList&values2,NodeList&rows2,NodeList&columns2)
    {
        ...
        {
            Node*Add;
            if(!Exist(cc->info)) {Add=new Node;Add->x=cc->info;
...
void algorithm(){
   FILE *fp,*fp2,*fp3; //указатели на файлы
    fp=fopen("fin.txt","rb"); //открываем конкретный файл
    fp2=fopen("fout.txt","wb");//открываем файл для записи бинарного кода
    fp3=fopen("binary.txt","wb");//открываем файл для записи сжатого файла
