...
void Create_array(string*arr,int&arr_counter,char*original),derivate(node*node);
bool IS_Oper(string current),IS_Func(string current),IS_Brackets(string current);
int Prior(string current);
void graph(node*node,FILE*my);
void viz(node*node,FILE*my);
node*Create_node(string information),*Prefix_form(string*arr,int arr_counter);void solve(stack<node*>&nodes,stack<node*>&oper);
