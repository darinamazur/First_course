...
int main(){
    FILE*ind=fopen("index.txt","r");char*c=new char[10];fgets(c,10,ind);index=atoi(c);fclose(ind);
    FILE*f=fopen("file.txt","r+b"),*s=fopen("nfile.txt","a+b");
    info temp;fread(&temp,sizeof(info),1,f);while(!feof(f)){fwrite(&temp,sizeof(info),1,s);fread(&temp,sizeof(info),1,f);} fclose(f);fclose(s);
    int answer;
    node*tree=NULL;
    node*ntree=NULL;
    do{
    ...
    switch(answer){
        ...
        case 1: {Counter=0;tree=nullptr;info*arr=file_to_arr();
    ...
    break;}
        ...
        case 2: {info*words=new info[Size];
    while(tree){
    int a=maxVal(tree);
    ...
    cout<<endl<<endl;system("pause");Counter=0;break;}
        ...
        case 4:{Counter=0;ntree=nullptr;info*narr=nfile_to_arr();
    cout<<"New tree:"<<endl;
        cout<<endl<<endl;
    for(int i(0);i<index;i++){
    ...
    break;}
        ...
        case 5: {info*nwords=new info[Size];
    while(ntree){
    int na=maxVal(ntree);
...
info*file_to_arr(){
    ...
    do
    {
    arr[i]=b;i++;fread(&b,sizeof(b),1,my2);}while(!feof(my2));
    fclose(my2);
    return arr;
}
info*nfile_to_arr(){
    ...
    do
    {
    arr[i]=b;i++;fread(&b,sizeof(b),1,my3);}while(!feof(my3));
    fclose(my3);
    return arr;
}
...
return tree;
 ...
 void fout(){
   info b;
    ...
    do
    {cout<<b.eng<<" "<<b.ru<<" "<<b.counter<<endl;fread(&b,sizeof(b),1,my1);}while(!feof(my1));
    fclose(my1);
 }
int MainMenu() {
	int key = 0;
